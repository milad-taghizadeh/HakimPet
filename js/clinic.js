...
// // Vaccine to AdminPanel
document.getElementById('VaccineSubmit').addEventListener('click', vaccineData);
function vaccineData() {
    const lastVaccineDate = document.getElementById('shamsi-date').value;
    console.log(document.getElementById('shamsi-date').value);
    const vaccineName = document.getElementById('VaccineType').value;
    ...
    fetch('http://localhost:3000/api/v0/vaccination/', {
        ...
        body: JSON.stringify(vaccineDate)
    })
        .then(response => response.json())
        .then(vaccineDate => {
            console.log('Success:', vaccineDate);
        })
        .catch(error => {
